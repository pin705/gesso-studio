export default defineEventHandler(() => {
  ...
  return listProjects().map((project) => {
    const counts = useDb().prepare(`SELECT COUNT(*) AS assets,
      ...
      FROM assets WHERE project_id = ?`).get(project.id, project.id) as { assets: number; approved: number | null; feedback: number };
    const cover = useDb().prepare("SELECT key FROM assets WHERE project_id = ? ORDER BY updated_at DESC LIMIT 4").all(project.id) as { key: string }[];
